...
class ListHewanAdapter (private val listHewan: ArrayList<Hewan>): RecyclerView.Adapter<ListHewanAdapter.ListViewHolder>(){
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ListViewHolder {
        val view: View = LayoutInflater.from(parent.context).inflate(R.layout.item_row_hewan, parent, false)
        return ListViewHolder(view)
    }

    override fun onBindViewHolder(holder: ListViewHolder, position: Int) {
        val (name, user, photo) = listHewan[position]
        holder.imgPhoto.setImageResource(photo!!)
        holder.tvName.text = name
        holder.tvDescription.text = user
